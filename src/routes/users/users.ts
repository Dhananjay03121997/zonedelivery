import { generateToken, getDataByEmail, validateEmail } from "../../helper/functions.js";
...
const userLogin = async (body) => {
    try {
        ...
        if (!data) {
            return { message: messages.notFound.replace('##name##', 'user'), code: 400 };
        }
...
const getUserList = async (queryParams) => {
    try {
        const { searchText, pageNumber, pageSize }: { searchText: String, pageNumber: number, pageSize: number } = queryParams;
        let whereCondition = {};
        if (searchText) {
        ...
        const userList = await user.find(whereCondition)
        ...
        return { code: 200, message: messages.notFound.replace('##name##', 'data') };
    } catch (error) {
        return { code: 500, error: error.message };
    }
}
